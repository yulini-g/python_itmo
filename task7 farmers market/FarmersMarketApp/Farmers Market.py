...
def read_markets(filename): # Читает CSV с табуляцией и возвращает список словарей.
    ...
    with open(filename, 'r', encoding='utf-8') as f:
        ...
        for row in reader:
            ...
            market_name = clean_row.get('MarketName', '').strip()

            if fmid and fmid.isdigit() and market_name:
...
def get_market_by_id(markets, fmid): # Находит рынок по ID.
    """
    >>> markets = read_markets('database.csv')
...
def sort_markets(markets, key, is_reversed, reviews): # Сортировка ранков по заданному ключу.
    ...
    for market in markets:
        if key == 'city_state': # Отбираем только рынки с заполненным ключом
            ...
            city = market.get('city', '').strip()
            if state and city:
                filtered.append(market)
        elif key == 'rating':
            filtered.append(market)
        else:
            value = market.get(key, '').strip()
            if value:
                filtered.append(market)
    ...
    sorted_data = sorted(filtered, key=get_key, reverse=is_reversed)
    return sorted_data
...
def get_sort_key(markets, reviews, reviews_file): # Определяет параметр, по которму будет происходить сортировка.
    ...
    print(f"\nСортировка по: {key_names.get(key, key)}")
    print(f"Порядок: {'по убыванию' if is_reversed else 'по возрастанию'}")
    print(f"Найдено рынков: {len(sorted_list)}")
    print("-" * 50)
...
def get_distance (markets, reviews, reviews_file):
    ...
    show_search_result(result, markets, reviews, reviews_file)
def show_search_result(result, markets, reviews, reviews_file): # Показывает результаты поиска с указанием параметров
    if not result:
        print('Ничего не найдено.')
        return
...
def pick_market(markets, action):
    ...
    while True:
        ...
        for i in range(len(on_page)):
            ...
            market = on_page[i]
            print(f"{index} | {market.get('MarketName', '')} | {market.get('city', '')} | {market.get('State', '')}")
        print('-' * 50)
    ...
    while True:
        ...
        if choice == '2':
            ...
            for m in new_markets:
                markets.append(m)
            save_markets('database.csv', markets)
...
def load_reviews(filename): # Загружает рецензии из json файла.
    ...
    >>> reviews = load_reviews('nonexistent.json')
    Файл с отзывами (reviews.json) не найден.
    Это нормально при первом запуске.
    Файл будет создан автоматически, когда вы оставите первый отзыв.
    >>> type(reviews)
    <class 'list'>
    >>> len(reviews)
    0
    ...
    try:
        ...
        print('Файл с отзывами (reviews.json) не найден.')
...
#================== ТЕСТИРОВАНИЕ =========================
# """ Файл с тестовыми отзывами будет удален автоматически! """
# if __name__ == '__main__':
#     import doctest
#     doctest.testmod(verbose=True)
# try:
#     import os
#     os.remove('tested_save.json')
